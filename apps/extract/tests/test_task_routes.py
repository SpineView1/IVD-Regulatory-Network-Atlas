...
def test_enqueue_pending_chunks_routed_to_io():
    ...
    assert (
        "extract.tasks.enqueue_pending_chunks" in routes
...
def test_smoke_all_models_routed_to_io():
    routes = settings.CELERY_TASK_ROUTES
    assert (
        "extract.tasks.smoke_all_models" in routes
...
def test_run_ppi_not_statically_routed():
    ...
    assert "extract.tasks.run_ppi" not in routes, (
        "extract.tasks.run_ppi must NOT have a static route; " "it is routed dynamically per model"
    )
